...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
                http.csrf().disable()
                .authorizeRequests()
                        .antMatchers("/admin/building-edit","/admin/building-edit-{id}","/admin/user-edit-{id}","/api/building/{ids}").hasAnyRole("MANAGER","STAFF")
                        .antMatchers("/admin/**").hasAnyRole("MANAGER","STAFF","ADMIN")
                        .antMatchers("/login", "/resource/**","/register", "/trang-chu", "/api/**").permitAll()
                .and()
